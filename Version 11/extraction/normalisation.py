...
def stemming(annotations) :
    ...
    stop_words.update([",","(","d'",")","l'"] ) #custom stop words
...
def createData(annotations2):
    ...
    return sorted(data, key=lambda x: x[3], reverse=True), ent_cat  
...
def get_all_sentences(path):
    ...
    #all_sentences_filtered = [sentence for sentence in all_sentences if sentence != '']   
